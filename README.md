...
A color scheme with minimal amount of highlighting for Neovim. Formerly known as **Alabaster**, it keeps the same palette inspired by the original [Sublime Text Alabaster theme](https://github.com/tonsky/sublime-scheme-alabaster).
...
- Minimal does not highlight standard language keywords (if, else, function, etc). They are usually the least important and most obvious part of any program.
...
## Configuration
Two config options are provided (legacy `g:alabaster_*` names are still read for compatibility):
- `g:minimal_dim_comments` (default: `false`). When true, comments are dimmed instead of being highlighted bright yellow.
- `g:minimal_floatborder` (default: `false`).
...
## Minimal palette for other tools
These exports keep the original "alabaster" filenames but share the same palette.
